...
Card::Card(const bool isActivated, 
           ...
           isExpired_(isExpired)
           {
            if (!isExpired_)
            {
            expiration_ = generateExpiryDate();  // valid future expiration date
            }
             else
              {
                expiration_ = "00/00";  // expired date
              }
                card_number_ = "NONE";  // or generate if needed
                cvv_ = "000";           // or generate if needed
           }
